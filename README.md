...
- 验证码通过 SMTP 发送，使用 HTML 邮件模板。当 `MAIL_USERNAME` / `MAIL_PASSWORD` 未配置时，开发模式会将验证码打印到后端控制台日志。
...
- 模拟面试 可以选择简历进行针对性面试（一个是目标年限的职位、一个是深挖项目细节）
- 简约会员体系 -兑换码 要有后台，在admin账户里，真实显示调用次数
